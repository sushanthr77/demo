...
func (t *SimpleChaincode) create_document(stub shim.ChaincodeStubInterface, caller string, caller_affiliation string,
	document_json string, documentType string) ([]byte, error) {
...
func (t *SimpleChaincode) Invoke(stub shim.ChaincodeStubInterface, function string, args []string) ([]byte, error) {
	...
	if function == "create_trade" {
		...
		return t.create_document(stub, caller, caller_affiliation, args[0], args[1])
...
func createDocument(document_json string, docType string) (DocumentInt, error) {
	switch docType {
	case DT_SMRY_INVOICE:
		var sInv SummaryInvoice
		err := json.Unmarshal([]byte(document_json), &sInv) // Convert the JSON defined above into a SummaryInvoice object for go
		if err != nil {
